...
# Returns whether more time than the timeout limit has passed since the last
# call to the query method.
#
# TODO: This system should probably be updated sometime, but for now, it works.
timedOut = ->
    -lastConnect + (lastConnect = Date.now()) > timeOutLimit

# Performs a query, first checking if the database needs to be reconnected. As
# with timedOut, this check could be improved upon.
#
...
# passing it to the given callback function.
#
...
# end of the data retrieval, one callback is called for each row returned, and
# then an optional final callback is called.
#
# * channel: the channel id of the channel whose data is being requested
# * table: the table containing the data being requested
# * callback: a function taking a single row from the table as an argument, to
#             be called multiple times
# * lastcb: a no-argument function to be called after all of the data has been
#           processed
...
# Simply dumps everything from a table onto a callback function, throwing any
# error encountered.
#
...
#           as the key in the result object, and a value attribute for the
#           value to map to
...
# Loads a bucket, i.e., a map of the values of the key column to their
# corresponding row, for a channel in the specified table, and passes it to a
# callback.
#
