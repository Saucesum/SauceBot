...
# 
# Notes:
# Channel ID is necessary to get pretty much any data, as almost all data is specific to a channel.
# It is also the preferred (and only) way to refer to a channel, so any argument specifying a channel should be given a channel ID.
...
# Returns whether more time than the timeout limit has passed since the last call to the query method.
# This system should probably be updated sometime, but for now, it works.
timedOut = ->
    -lastConnect + (lastConnect = Date.now()) > timeOutLimit

# Performs a query, first checking if the database needs to be reconnected. As with timedOut, this check could be improved upon.
# * args: the arguments to pass to the MySQL client query function; in order, the SQL statement, the parameters to the query, and a callback function of the form (error, results) ->
# Note: only the statement argument is necessary, with the parameters being potentially optional, and the callback always optional
query = (args...) ->
    connect() if timedOut()
    client.query args...


# Loads all of the data for a specific channel from a specified table, then passing it to the given callback function.
...
# Similar to getChanData, but instead of having a single callback called at the end of the data retrieval, one callback is called for each row returned, and then an optional final callback is called.
# * channel: the channel id of the channel whose data is being requested
# * table: the table containing the data being requested
# * callback: a function taking a single row from the table as an argument, to be called multiple times
...
# Simply dumps everything from a table onto a callback function, throwing any error encountered.
...
# A variation of getData, akin to getChanDataEach, that first fetches the data, then passes each row sequentially to a callback, and then executes an optional final callback.
# * table: the table containing the data being requested
# * callback: a function taking a single row from the table as an argument, to be called multiple times
# * lastcb: a no-argument function to be called after all of the data has been processed
...
# Deletes all data in a given table about a specified channel id, running a callback on completion.
...
# Removes all rows in a given table for a specified channel where the column specified by 'field' matches 'value', running a callback on completion.
...
# Generates a string of comma-separated wildcard variables of a given length for use in queries
# * num: the number of variables to generate
getWildcards = (num) ->
    ('?' for [1..num]).join ', '


# For each row in datalist, adds a row containing the given fields set to the values in the datalist row, for a given channel, to the specified table, or replaces the row if it already exists.
...
# Performs the same function as addChanData, but first clears all data for the channel in the specified table.
# * channel: the channel which the data should be added to
# * table: the table to add the data to
# * fields: the columns whose values are being set in the new rows
# * datalist: the actual data of each row, labeled by fields
...
# Loads, for a given channel, either all of the values from a table in a specified column, or a mapping of the values in one "key" column to those in another column, and then passes the result to a callback.
# This can be thought of as either allowing JS to automatically index the data, or providing our own index, respectively.
# * channel: the channel to filter our data to
# * table: the table containing the desired data
# * fields: either the name of a column in the table, if we just want an array; or an object with a key attribute specifying the column to be used as the key in the result object, and a value attribute for the value to map to
...
# Loads a bucket, i.e., a map of the values of the key column to their corresponding row, for a channel in the specified table, and passes it to a callback.
