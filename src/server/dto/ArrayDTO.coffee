...
# In the database, the array takes the form of a set of rows (chanid, value) such as (1, "red"), (1, "blue") in order to form the array ["red", "blue"] for channel 1.
...
class ArrayDTO extends DTO
    ...
    load: (cb) ->
        # Here we use load data to simply load all of the values of the column where the row contains the matching channel ID
    ...
    save: ->
        # Because loadData is asymmetric, we have to use setChanData to store the data, by calling it to only store rows with the value field set to the stored data
    ...
    add: (item) ->
         ...
         @data.push item
         # To store the new item, we have to add a row containing the new item as the value attribute to the table, so the set of rows is [[item]]
